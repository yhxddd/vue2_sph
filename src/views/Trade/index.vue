<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressArray" :key="address.id">
        <span class="username" :class="{selected: address.isDefault == 1}">{{address.consignee}}</span>
        <p @click="changeDefault(address,addressArray)">
...
<script>
  export default {
    ...
    computed:{
      addressArray(){
        return this.$store.state.tradeModule.addressArray;
      },
      orderInfo(){
        return this.$store.state.tradeModule.orderInfo;
      },
      // 提交订单选中的地址  find:查找符合条件的内容
      userDefaultAddress(){
        return this.addressArray.find(item => item.isDefault == 1 ) || {};
      }    
    },
    methods:{
      // 修改默认地址
      changeDefault(address, addressArray){
        // 全部的isDefault为0
        addressArray.forEach(item => item.isDefault = 0);
        address.isDefault = 1;
      },
      async submitOrder(){
        // 订单编号
